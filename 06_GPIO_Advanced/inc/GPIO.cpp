...
int GPIO::waitForEdge()
{
	...
	if (epollfd == -1)
	{
		std::perror("GPIO: Failed to create epollfd");
		return -1;
	}
	...
	if (fd == -1)
	{
		std::perror("GPIO: Failed to open file");
		return -1;
	}
	...
	{
		std::perror("GPIO: Failed to add control interface");
		return -1;
	}
	...
	if (close(epollfd))
	{
		std::perror("Failed to close epoll file descriptor");
		return -1;
	}
